...
drop table if exists {zona_p}.{prefijo}_sabana_test purge;



create table if not exists {zona_p}.{prefijo}_sabana_test stored as parquet tblproperties ('transactional'='false') as
    ...
    with master as (
        ...
        select
            ...
            t2.tot_patrimonio,
            t2.smmlv
        from proceso_ecosistemas.opeco_sabana_test_hist_pagos as t1
    ...
    select 
        ...
        t2.prob_auto_cura
    from master_test_hist_pagos as t1
    left join prob_h as t2
    on 
        t1.key = t2.nit_oblig_fecha